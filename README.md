...
### 2. 启动小鸡
```bash
# 用法: ./deploy-nat.sh <密码> <镜像类型> [CPU核心数] [内存MB]
./deploy-nat.sh MyPass123 debian 1 512
```

**资源限制说明:**
- **默认/推荐**: 脚本会自动限制资源以保证宿主机稳定。
- **Debian**: 内存建议最小 `512MB`。
- **Alpine**: 内存建议最小 `128MB`。
- 如果输入的内存低于建议值，脚本会自动调整为建议最小值。

**自动化逻辑 (10000/20000 分离):**
- **SSH 端口**: 自动从 `10000` 段寻找首个空闲端口。
- **NAT 端口**: 自动从 `20000` 段寻找连续的 `100` 个空闲端口。
- **独立搜寻**: SSH 端口与 NAT 端口独立查找，互不交叉，确保不会冲突。
- **示例**: 分配到 SSH 端口 `10001`，NAT 端口段可能分配到 `20000-20099`。
